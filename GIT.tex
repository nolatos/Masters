...
\begin{definition}
	Let $G$ be a reductive affine algebraic group acting on $X \subseteq \P^n$, with a linearisation on $\O_{X}(1)$. Then we have a linear action of $G$ on $\widetilde{X}\subseteq \A^{n+1}$. Now given a 1-PS $\lambda: \bb{G}_m \rightarrow G$, we have a weight space decomposition \[ k^{n+1}=: V = \bigoplus_{r\in \Z} V_r. \] Choose a basis $\{e_i\}$ for $V$ such that $\lambda \cdot e_i = \lambda^{r_i} e_i$ for all $\lambda\in \bb{G}_m(k)$. Now let $p\in X(k)$, and let $\tilde{p}\in \widetilde{X}(k)$ be a lift. We may write $\tilde{p} = \sum p_i e_i$. The \textit{Hilbert-Mumford weight of $\lambda$ at $p$}, denoted $\mu(p, \lambda)$, is the integer \[\mu(p, \lambda) := \max\{-r_i \mid p_i \neq 0\}.\] Note that this does not depend on the choice of $\tilde{p}$.
\end{definition}
We are now in a position to state the theorem:
\begin{theorem}[The Hilbert-Mumford Criterion]
	Let $p$ be a $k$-point. Then $p$ is semistable if and only if $\mu(p, \lambda)\geq 0$ for all nontrivial 1-PS $\lambda$, with stability holding if and only if the condition holds with strict inequality.
\end{theorem}
\begin{proof}
	\autocite[pp. 49-54]{GIT}
\end{proof}
Let us break down the theorem a little. The \enquote{only if} is actually quite intuitive: let $\lambda$ be a 1-PS. If $\mu(p, \lambda) < 0$, this means that all the $r_i$ in the preceding definition are positive. In particular, \[\lim_{t\to 0} \lambda_p(t) = 0, \] and so $p$ is unstable. Conversely, if $\lim_{t\to 0} \lambda_p(t) = 0,$ then it must be the case that all the $r_i$ are positive. Hence $p$ is unstable for this induced $\bb{G}_m$-action if and only if $\mu(p, \lambda) < 0$. Now if $\mu(p, \lambda) = 0$, then all the $r_i$ are nonnegative, with at least one strictly zero, and so the limit does exist, and it easy to see \[\lim_{t\to 0} \lambda_p(t) = \sum_{r_i = 0}  p_i e_i. \] In particular, if $\lambda$ is not trivial, then either the limit lies outside the orbit, whence $p$ is not polystable, and thus not stable, or the 1-PS is contained in the stabiliser $G_p$, whence $p$ is still not stable. If all the weights are nonpositive, the same argument can be made for $\lambda^{-1}$ in place of $\lambda$, and so in order for $p$ to be stable, there must be a mixture of positive and negative weights. 

The significance of this theorem is that the \enquote{if} is also true: in particular it is saying that if $p$ is unstable (or not stable), then there is a 1-PS which can detect this, which is not at all obvious. Moreover, it is also saying that if the stabiliser $G_p$ of $p$ is positive dimensional, then there is a nontrivial 1-PS $\lambda: \bb{G}_m \rightarrow G_p$.
