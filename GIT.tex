...
\end{example}
\begin{example}
	As we saw in Theorem \ref{torus-reductive}, $\bb{G}_m(k)$ is reductive.
\end{example}
\begin{example}
...
\end{proposition}
\begin{proof}
	Firstly, let $V$ be an irreducible finite dimensional subrepresentation. Then $aV$ is also irreducible, and $v \mapsto av$ is a $k[G(k)]$-module homomorphism, and thus by Schur's lemma, is either trivial or an isomorphism. 
...
\begin{proof}
	...
	Finally, to prove (iii), it suffices to show that if $p$ is polystable then $\dim G_p = 0$ if and only if $\dim G_{\tilde{p}} = 0$. Clearly $\dim G_{\tilde{p}} \leq \dim G_p$, and so it suffices to show that if $\dim G_{\tilde{p}} = 0$ then $\dim G_p = 0$. To this end, observe that {\color{red} TODO }.
\end{proof}
...
%\end{proof}
\begin{lemma}
	Let $G$ be an affine algebraic group and let $\lambda: \bb{G}_m \rightarrow G$ be a 1-PS. Then $\lim_{t \to 0} \lambda(t)$ exists if and only if $\lambda$ is trivial.
\end{lemma}
\begin{proof}
	If $\lambda$ is trivial then the limit exists trivially. Conversely, suppose the limit exists, that is to say the ring homomorphism $\lambda^\sharp:\Gamma(G,\O_G) \rightarrow k[t^{\pm 1}]$ factors through $k[t] \rightarrow k[t^{\pm 1}]$. In particular, the image of $\lambda^\sharp$ cannot intersect the ideal $\langle t \rangle$ in $k[t]$, since if $\lambda^\sharp(f) = Ft$, then $\lambda^\sharp(\iota^\sharp(f)) = \iota^\sharp(\lambda^\sharp(f)) = \iota^\sharp(F)t^{-1}$, and hence the homomorphism cannot factor through $k[t]$. Therefore the image of $\lambda^\sharp$ is contained in the image of $k$ in $k[t^{\pm 1}]$, and we can show using the group axioms that $\lambda^\sharp$ must factor through $e^\sharp$, as desired.
\end{proof}

Let $G$ be an affine algebraic group acting on a quasi-projective variety $X$, and fix a 1-PS $\lambda: \bb{G}_m \rightarrow G$. We have an induced morphism $\lambda_p: \bb{G}_m \rightarrow \A^{n+1}$, and if the limit at 0 does exist, then either the limit is $p$ (in which case the 1-PS acts trivially), or the limit is in the complement of $G\cdot p$ in its closure, {\color{red} by the above lemma}. In particular, the stability of $p$ is encoded in whether or not the limit exists. The idea is now to find a numerical criterion for determining whether the limit will exist; this is done using the following definition: %This is clear. What is not clear is that the converse is also true:
...
\end{proof}
Finally, we may state the theorem:
\begin{theorem}[The Hilbert-Mumford Criterion]
	Let $p$ be a $k$-point. Then $p$ is semistable if and only if $\mu^{\Ox(1)}(p, \lambda)\geq 0$ for all nontrivial 1-PS $\lambda$, with stability holding if and only if the condition holds with strict inequality.
\end{theorem}
\begin{proof}
	\autocite[pp. 49-54]{GIT}
\end{proof}
...
The significance of this theorem is that the \enquote{if} is also true: in particular it is saying that if $p$ is unstable (or not stable), then there is a 1-PS which can detect this, which is not at all obvious. Moreover, it is also saying that if the stabiliser $G_p$ of $p$ is positive dimensional, then there is a nontrivial 1-PS $\lambda: \bb{G}_m \rightarrow G_p$. These follow from a theorem of Iwahori relating 1-PS's to $k((t))$-valued points of $G$. The details are found in the citation in the proof.
...
\begin{definition}
	Let $k$ be an algebraically closed field of characteristic zero. The \textit{moduli problem of conics in $\P^2$ up to projective transformations} is the functor \[ \M^\flat: \mathsf{LNSch}/k \rightarrow \Sets, \; S\mapsto \{\text{families over } S\}/\Aut_S(\P^2\times S). \]
\end{definition}
