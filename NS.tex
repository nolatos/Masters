\par {\color{red} We have constructed the spaces $V^s_{n,d}$ of stable vector bundles on a curve. In this chapter, we turn our attention to the case $k = \C$ (in particular, our vector bundles are \textbf{complex} vector bundles). This choice is necessary, as it gives us access to methods from analysis and differential geometry. In this setting, it turns out $V^s_{n,d}$ has a nice characterisation, known as the \textit{Narasimhan-Seshadri Correspondence}, or the \textit{Narasimhan-Seshadri Theorem}. }So let us fix once and for all a compact Riemann surface $X$ of genus $g$ (which may be identified as the $\C$-valued points of a projective curve over $\C$, given the complex topology). The basic result is the following:
\begin{theorem}[Narasimhan-Seshadri, 1965]
	Suppose $g\geq 2$. Then there is a bijection between $V^s(n,0)$ with irreducible representations $\pi_1(X) \rightarrow U(n)$ up to conjugation.
\end{theorem}
...
\end{theorem}
We will survey these results in this chapter.
\\\\
\indent Of course, since we are moving into analytic territory, some comments are in order. \textit{Open} and \textit{closed} will always mean with respect to the usual topology. \textit{Point} will always refer to a usual point. We will make use of the correpondence between vector bundles and locally free sheaves without comment, and we will also use without comment the correspondence between holomorphic vector bundles on $X$ and algebraic vector bundles on $X$. 
\section{Parameterising Holomorphic Structures}
\indent Within the problem of classifying holomorphic bundles, a subproblem is to classify all holomorphic bundles that restrict to a given smooth bundle $E$. As we will see in the next section, the degree of a holomorphic bundle is actually a smooth invariant, and in fact, along with the rank, completely classifies $E$! Thus $V_{n,d}$, the isomorphism classes of all holomorphic bundles with signature $(n,d)$, is equal to the set of holomorphic structures on $E$, up to isomorphism. It turns out that this space is, in turn, parameterised by so-called unitary connections (to be defined) on $E$, and this correspondence, known as the \textit{Chern Correspondence}, is a key ingredient in the Narasimhan-Seshadri theorem. We will study the Chern corespondence in this section.
...
We conclude this section with a discussion about the Sobolev completions of various spaces we have been working with, and how the above bijection extends. To begin, we first recall that since $X$ is a Riemann surface, it is K\"ahler with its Fubini-Study form. This fixes a volume form $\vol$ and hence a Hodge star product $\star$. The \textit{Hodge inner product} is defined on $\Omega^p_X$ as \[\langle\alpha, \beta\rangle := \int_X \alpha \wedge \star \beta. \] Completing this with 
\section{Chern Classes}
In this section, we will study Chern classes (or more precisely, the first Chern class), which are smooth invariants which are intimately connected with the notion of degree, and uniquely define the bundle up to smooth isomorphism. Once again, we fix a smooth bundle $E$. We begin with a result:
\begin{lemma}
...
\begin{theorem}\label{chern-weil-degree}
	For any holomorphic bundle $\E$ with underlying smooth bundle $E$, we have \[\int_X c_1(E) = \deg(\E)\] up to sign. A choice of orientation fixes the sign.
\end{theorem}
...
\\\\
\par Before we proceed, we would like to review the Snake Lemma; in particular how the \enquote{snake} is constructed. We recall the statement:
...
\begin{equation*}
	...
	\end{tikzcd}
\end{equation*}
where $\exp$ above is the map $x \mapsto \exp(2\pi i x)$. Taking the sheaf of $\scr{P}$-functions with values in the above groups, we have the following short exact sequence of sheaves, known as the \textit{exponential sheaf sequence}: 
...
\begin{theorem}\label{chern-weil-degree-line}
	If $\L$ is a holomorphic line bundle with underlying smooth bundle $L$, then up to sign we have:
	\begin{equation}\label{chern-degree-equation-line}
		\int_X c_1(L) = \deg \L
	\end{equation}
	A choice of orientation fixes our sign.
\end{theorem}
\begin{proof}
	We first prove the case where $\L$ is the line bundle of a prime divisor. Let $D$ be a prime divisor, supported on $P\in X$, suppose $\L = \L(D)$. Since $D$ is effective, by Proposition \ref{divisor-of-zeroes-effective}, it is the divisor of zeroes of some global section $s\in H^0(X, \L)$ that vanishes exactly once, at $P$. In particular, $s$ is a frame on $X \setminus \{P\}$. Now fix a hermitian metric $h$. By the Chern correspondence, the Chern connection of $(\E, h)$ is given locally by $h^{-1}_\alpha\partial h_\alpha$ and the curvature by $dh^{-1}_\alpha \partial h_\alpha= d\partial \log h_\alpha$ (here we may take logarithms because $h_\alpha > 0$). Now for an $\varepsilon> 0$, write $U_\varepsilon$ for the open set $X \setminus \overline{B}_{\varepsilon}(P)$, define $s_\varepsilon$ to be the frame $s|_{U_\varepsilon}$ and finally write $h_\varepsilon:= h(s_\varepsilon, s_\varepsilon) > 0$. We compute: \[\int_X c_1(L) = \lim_{\varepsilon \to 0} \int_{U_\varepsilon}c_1(L) = \lim_{\varepsilon \to 0} \int_{U_\varepsilon}\frac{i}{2\pi} d\partial \log h_\varepsilon = \lim_{\varepsilon \to 0}\int_{\partial U_\varepsilon} \frac{i}{2\pi}\partial \log h_\varepsilon \] where the final equality follows from Stoke's theorem. Now since $s|_{B_\varepsilon(P)}$ vanishes exactly at $P$, we may pick a holomorphic coordinate chart centred at $P$ such that $s = z$. Hence \[\int_{\partial U_\varepsilon} \partial \log h_\varepsilon= \pm \int_{|z| = \varepsilon} \partial (\log z + \log \bar{z} + \log h_z(1,1)) \] with the sign depending on our choice of orientation. Now observe that since $\log h_z(1,1)$ is smooth, it follows $\partial \log (h_z(1,1))/\partial z$ is continuous, and since $\{|z| \leq \delta \}$ is compact, for some sufficiently small $\delta$, it follows $\partial \log (h_z(1,1))/\partial z$ is bounded on $\{|z| \leq \delta \}$. Thus \[\lim_{\varepsilon \to 0}\left| \int_{|z| = \varepsilon} \partial \log h(1,1)\right| = \lim_{\varepsilon \to 0} \left|\int_{|z| = \varepsilon} \frac{\partial \log h_z(1,1)}{\partial z} dz\right| \leq \lim_{\varepsilon\to 0} 2\pi \varepsilon \sup\{ \left|\frac{\partial \log h_z(1,1)}{\partial z}\right| \mid |z|\leq \delta \} = 0. \] Finally, we have, by Cauchy's integration formula, \[\int_{|z| = \varepsilon} \partial (\log z + \log \bar{z}) =  \int_{|z| = \varepsilon} \frac{dz}{z} = 2\pi i  \] as desired. 
...
\end{proof}
To make further arguments easier, we will henceforth choose the orientation such that equality holds as in (\ref{chern-degree-equation-line}).
...
\begin{proof}[Proof of Theorem \ref{chern-weil-degree}]
	By Theorem \ref{chern-weil-degree-line}, it suffices to show that there is some connection $\{\omega_\alpha\}$ on $E$, such that $\{\tr \omega_\alpha\}$ is a connection on $\det E$. To this end, we first observe that $E$ can be decomposed into a direct sum of line bundles (for example, letting $L$ be a subbundle, fixing a hermitian metric on $E$ and taking the unitary complement of $L$ induces a splitting $E = L \oplus L^\perp$; and repeating with $L^\perp$ we deduce the result). So let \[E = \bigoplus_{1 \leq i \leq n} L_i \] be such a decomposition, and for each $L_i$ we fix a connection $\{\omega_\alpha^i\}$ on each $L_i$, relative to the frames $\{s_{i, \alpha}\}$. Now observe that $s_\alpha:=(s_{1, \alpha},...,s_{n, \alpha})$ is a frame on $E$, and clearly we have an induced connection described by $\omega_\alpha:= \diag(\omega_\alpha^1,...,\omega_\alpha^n)$ with respect to the frame $s_\alpha$. Now the transition functions are also diagonal matrices described by $\diag(g_{\alpha\beta, 1},...,g_{\alpha\beta, n})$ where $g_{\alpha\beta, i}$ is the relevant transition function on $L_i$. Now we compute. By Proposition \ref{connection-transformatino-rule},  it suffices to show \[\tr(\omega_\beta) = \det g^{-1}d(\det g) + \tr(\omega_\alpha)  \] for any two $\omega_\alpha, \omega_\beta$. To this end, simply note \[\det g^{-1}d(\det g) + \tr(\omega_\alpha) =( \prod_{1\leq i \leq n} g_i^{-1})(\sum_{1 \leq i \leq n} dg_i\prod_{j\neq i} g_j) + \sum_i \omega_{\alpha}^i = \sum_i g_i^{-1}dg_i + \omega_\alpha^i = \sum \omega_\beta^i = \tr \omega_\beta \] as desired.
\end{proof}
Finally, we need to prove Theorem \ref{chern-weil-degree} and the fact that the Chern class (and hence degree) classifies all smooth bundles up to diffeomorphism. Both follow immediately from the following result:
\begin{theorem}[Structure Theorem of Smooth Vector Bundles]
	There is a diffeomorphism \[E \cong \det E \oplus \Ox^{\rk E - 1}. \]
\end{theorem}
\begin{proof}
	It suffices to prove that every bundle of rank greater than one has a nonvanishing global section. {\color{red} This follows form the fact that there always a section transverse to the zero section, and by dimensionr reasons, these sections must not intersect}.
\end{proof}
\begin{corollary}
	For any holomorphic bundle $\E$ with underlying smooth bundle $E$, we have \[\int_X c_1(E) = \deg(\E).\] 
\end{corollary}
\begin{proof}
	If $\nabla$ is a connection on $\det E$ with local 1-forms $\{\omega_\alpha\}$, then $\diag(\omega_\alpha,0,...,0)$ is the matrix of 1-forms for a connection on $\det E \oplus \Ox^{\rk E - 1}$. Thus \[\int_X c_1(E) = \int_X c_1(\det E) = \deg(E) \] as desired.
\end{proof}
\begin{corollary}
