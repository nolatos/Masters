...
To summarise, we have given an interpretation of $V^s_{n,d}(\C)$, the set of $\C$-points of $V^s_{n,d}$ as the set of holomorphic structures on the unique smooth bundle $E$ of signature $(n,d)$. In particular, our earlier work on the Chern correspondence may be interpreted as giving a map from $V^s_{n,d}(\C)$ to the space of unitary connections modulo gauge equivalence. This observation will be key in the sequel.
\input{RH}
\input{Donaldson}